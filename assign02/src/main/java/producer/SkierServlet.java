...
import producer.pool.ChannelPooledFactory;
...
//            this.pool.setMaxTotal(-1);  // set pool size to infinite
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws IOException {
        ...
        if (!urlParts[EMPTY_PARAMETER_INDEX].equals("") || !isValidPostParameters(urlParts) || !isValidJsonBody(requestString)) {
            ...
            response.setStatus(HttpServletResponse.SC_OK);
            // do any sophisticated processing with urlParts which contains all the url params
            final int SKIER_ID_INDEX = 7;
            jsonResponse = gson.toJson(new Skier(Integer.parseInt(urlParts[SKIER_ID_INDEX]), gson.fromJson(requestString, LiftRide.class)));
    ...
    private boolean isValidJsonBody(String requestString) {
        try {
            gson.fromJson(requestString, LiftRide.class);
        } catch (Exception e) {
            e.printStackTrace();
            return false;
        }
